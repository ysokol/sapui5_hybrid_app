sap.ui.define([
	...
	return BaseObject.extend("my.sapui5_hybrid_app.model.GroupSortState", {
		 ...
		 * @alias my.sapui5_hybrid_app.model.GroupSortState
		 */
		...
		/**
		 * Sorts by SalesOrder, or by OrderTotalGross
		 *
		...
		/**
		 * Groups by OrderTotalGross, or resets the grouping for the key "None"
		 *
		 * @param {string} sKey - the key of the field used for grouping
		 * @returns {sap.ui.model.Sorter[]} an array of sorters
		 */
		group: function (sKey) {
			...
			if (sKey === "OrderTotalGross") {
				// Grouping means sorting so we set the select to the same Entity used for grouping
				this._oViewModel.setProperty("/sortBy", "OrderTotalGross");

				aSorters.push(
					new Sorter("OrderTotalGross", false,
						this._fnGroupFunction.bind(this))
				);
			} else if (sKey === "None") {
				// select the default sorting again
				this._oViewModel.setProperty("/sortBy", "SalesOrder");
			}
